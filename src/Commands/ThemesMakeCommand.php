...
class ThemesMakeCommand extends Command
{
    ...
    private function createCssFile($theme, $panel, $panelId, $name): int
    {
        ...
        if (! file_exists(base_path('vite.config.js'))) {
            $this->components->warn('Action is required to complete the theme setup:');
            $this->components->bulletList([
                "It looks like you don't have Vite installed. Compile `resources/css/filament/{$panelId}/themes/{$theme}.css` into `public/css/filament/{$panelId}/themes/{$theme}.css` with the Tailwind v4 CLI:",
                ...
                "Register the theme in the {$theme} class inside `modifyPanelConfig` using `->theme(asset('css/filament/{$panelId}/themes/{$theme}.css'))`",
            ]);
